...
static int exynos4_usb_phy1_suspend(struct platform_device *pdev)
{
	...
	writel(phypwr, EXYNOS4_PHYPWR);
	usb_phy_control.status = USB_PHY_L2;
	return 0;
}
...
static int exynos4_usb_phy1_resume(struct platform_device *pdev)
{
	...
	if(USB_PHY_L0 == usb_phy_control.status)
		return 0;
    //printk("[ exynos4_usb_phy1_resume]++++++++++++++\n");
    if (exynos4_usb_host_phy_is_on()) {
		...
		if (usb_phy_control.lpa_entered) {
			...
			err = 0;//diog.zhao return to indicate from l3->l0
		} else
			err = 0;
	} else {
		phypwr = readl(EXYNOS4_PHYPWR);
...
static int exynos4_usb_phy20_exit(struct platform_device *pdev)
{
	...
	if (atomic_dec_return(&host_usage) > 0) {
		dev_info(&pdev->dev, "still being used\n");
		//if(smm6260_get_active_state())
			return -EBUSY;
	}
	if(USB_PHY_L3== usb_phy_control.status)
		return;
	...
	usb_phy_control.lpa_entered = 0;
	usb_phy_control.status = USB_PHY_L3;
	return 0;
}
...
extern int usb_check_curchage_status();
...
/*diog.zhao,0614
**only check vbus is high or not,
**it is equated to charge status is high*/
//	if (exynos4_usb_device_phy_is_on()) {
	if(usb_check_curchage_status()){
		op = 1;
		goto done;
	...
	if (soc_is_exynos4210()) {
		...
		if (phypwr & (PHY1_STD_FORCE_SUSPEND
			...
			if( USB_PHY_L3 == usb_phy_control.status)
				return;
#ifdef CONFIG_SMM6260_MODEM//diog.zhao 120605
