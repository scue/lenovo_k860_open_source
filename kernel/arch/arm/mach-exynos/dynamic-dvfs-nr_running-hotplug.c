...
static unsigned int early_suspended;		/* jeff, after early suspend, cpu3 down only depend on DVFS, otherwise it is hard to enter LPA */
		...
		if ((cpu_online(1) == 1) && (nr_running() < 4) &&
		   ((freq_old <= freq_out_trg) && (freq_new <= freq_out_trg))) {
			...
			if ((ctn_nr_running_under3 >= 8) &&
			  ...
			  ((freq_old <= freq_out_trg) && (freq_new <= freq_out_trg))) {
			if (((ctn_nr_running_under2 >= 8 || early_suspended) )&& /*jeff, not consider nr_running*/
			   (ctn_freq_out_trg_cnt >= 5)) {
				/* over 800ms for nr_running(), over 500ms for frequency, tunnable */
				cpu_down(3);
				ctn_freq_out_trg_cnt = 0;
...
{
	//printk("dvfs_nr_running: %s\n", __func__);
	early_suspended = 1;
}

static void hp_late_resume(struct early_suspend *h)
{
	//printk("dvfs_nr_running: %s\n", __func__);
	early_suspended = 0;
}
...
#ifdef CONFIG_HAS_EARLYSUSPEND
    early_suspended = 0;
	early_suspend_handler.suspend = hp_early_suspend;
	early_suspend_handler.resume = hp_late_resume;
	early_suspend_handler.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 50; //latest? later then LCD
