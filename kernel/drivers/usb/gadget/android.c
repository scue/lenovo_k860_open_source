...
static void android_work(struct work_struct *data)
{
	...
	char **uevent_envp = NULL;
	unsigned long flags;
	int charger_gpio = EXYNOS4_GPX2(4);
	int popup = 0;
	...
	if (cdev->config)
		popup = 1;
	if((!dev->sw_connected)&&(!gpio_get_value(charger_gpio)))
		popup = 2;
			       ...
			       const char *buff, size_t size)
{
	...
	char buf[256], *b;
	int err;
	printk("android_usb: %s++\n",__func__);
	mutex_lock(&dev->mutex);

	if (dev->enabled) {
		mutex_unlock(&dev->mutex);
		pr_err("android_usb: usb is already enabled, can't write functions\n");
		return -EBUSY;
	}
	...
	b = strim(buf);
	if(b)
		printk("android_usb: %s, b = %s\n",__func__,b);
	else
		printk("android_usb: %s, b = null\n",__func__);
	while (b) {
		name = strsep(&b, ",");
		if (name) {
			printk("android_usb: %s, name = %s\n",__func__,name);
			    ...
			    const char *buff, size_t size)
{
	...
	printk("android_usb: %s, enabled = %d\n",__func__,enabled);
...
android_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *c)
{
	...
	if (!dev->connected) {
		dev->connected = 1;
		printk("usb-work::connect\n");
		schedule_work(&dev->work);
	}
	else if (c->bRequest == USB_REQ_SET_CONFIGURATION && cdev->config) {
		printk("usb-work::configuration\n");
		schedule_work(&dev->work);
	}
	spin_unlock_irqrestore(&cdev->lock, flags);
