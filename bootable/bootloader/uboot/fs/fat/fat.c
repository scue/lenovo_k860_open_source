...
int fatlist_n;
char fatlist[FATLIST_MAX][FATNAME_LEN];
...
#if 1
#define MAXIMUM_BLOCK_COUNT 65535
                        int n = 0;
                        void *addr_to_read = bufptr;
			u32 cnt_to_read;
			u32 read_cnt;
			u32 blk = startblock;
			u32 cnt = getsize;
                        do {
                                ...
                                read_cnt = cur_dev->block_read(cur_dev->dev, blk, cnt_to_read, addr_to_read);
                                n += read_cnt;
                                blk += read_cnt;
				...
				if ((dentptr->attr & ATTR_VFAT) &&
					...
					if (dols) {
						...
						if (doit) {
							...
							if(fatlist_n < FATLIST_MAX) sprintf(fatlist[fatlist_n++], "%s", l_name, dirc);
						}
						dentptr++;
						continue;
					}
			...
			if (dols) {
				...
				if (doit) {
					if (dirc == ' ') {
					 ...
					 if(fatlist_n < FATLIST_MAX) sprintf(fatlist[fatlist_n++], "%s", s_name, dirc);
				}
