...
{	
	unsigned int reg;	
	reg = readl(0x10020710) & ~(1 << 0);	
	reg |= (enable << 0);	
	writel(reg, 0x10020710);
}

static void s5p_dsim_enable_dsi_master(unsigned char enable)
{	
	unsigned int reg;	
	reg = readl(0x10020710) & ~(1 << 2);	
	reg |= (enable << 2);	
	writel(reg, 0x10020710);
}
...
{	
	writel(S5P_MIPI_M_RESETN, 0x10020710);
}

void s5p_dsim_init_d_phy(unsigned int dsim_base)
{	
	printf("s5p_dsim_init_d_phy\n");
	s5p_dsim_part_reset();
	/* enable D-PHY */	
	...
	unsigned short hori_resol)
{
	...
	printf("%s:vert_resol:%d, hori_resol:%d\n", __func__, vert_resol, hori_resol);
	...
	struct dsim_lcd_config *main_lcd, struct dsim_lcd_config *sub_lcd)
{
	...
	if (main_lcd->e_interface == DSIM_VIDEO)
		...
		printf( "this ddi is not MIPI interface.\n");
		return;
	}
...
unsigned char s5p_dsim_is_lane_state(unsigned int dsim_base, unsigned char lane)
{
	...
	if ((lane & DSIM_LANE_ALL) > DSIM_LANE_CLOCK) { /* all lane state */
		...
		if (reg & (lane << 4)) {
			...
			printf("data lane state is unknown.\n");
			return -1;
		}
		...
		if (reg & (1 << 9)) {
			...
			printf("clock lane state is unknown.\n");
			return -1;
		}
	}
	...
	unsigned char state)
{
	...
	if (state == DSIM_STATE_HSCLKEN && data == DSIM_TRANSFER_BYLCDC)
		reg &= ~data;
	else
		reg |= data;
