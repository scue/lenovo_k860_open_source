...
/* RK_ID:Browser DEP_RK_ID:NULL. AUT:makun2@lenovo.com. DATE:2012-05-05. START */
/**
*To fix bug 167039
*/
        String patchedParseString = patchMetaElementEnd(parseString);
        //xmlParseChunk(context->context(), reinterpret_cast<const char*>(parseString.characters()), sizeof(UChar) * parseString.length(), 0);
        xmlParseChunk(context->context(), reinterpret_cast<const char*>(patchedParseString.characters()), sizeof(UChar) * patchedParseString.length(), 0);
/* RK_ID:Browser DEP_RK_ID:NULL. AUT:makun2@lenovo.com. DATE:2012-05-05. END */
...
*To fix bug 167039
*/
String XMLDocumentParser::patchMetaElementEnd(const String &from) {
    String decoded = String(from);
    ...
    found_meta = decoded.find(target);
    if(found_meta != WTF::notFound) {
        found_end = decoded.find(end,found_meta);
        if(found_end != WTF::notFound) {
            if(decoded[found_end - 1] != '/') {
                decoded.insert(miss,found_end);
            }
        }
    }
    return decoded;
...
void XMLDocumentParser::internalSubset(const xmlChar* name, const xmlChar* externalID, const xmlChar* systemID)
{
    ...
    if (document()) {
        ...
        /*if (isWMLDocument()
            ...
            // added by liumm2@lenovo.com 2012-01-09
            && extId != "-//OPENWAVE.COM//DTD WML 1.3//EN"
            // end
            )
            handleError(fatal, "Invalid DTD Public ID", lineNumber(), columnNumber());*/
        /***RK_ID:RK_WEBKIT. DEP_RK_ID:NULL. AUT:wangwf1@lenovo.com. DATE:2012-03-12. START***/
        if (isWMLDocument()
            /*** fix bug some web page not standard ***/
            && (!(extId.upper().contains("-//WAPFORUM//DTD")
                    || extId.upper().contains("-//OPENWAVE.COM"))
                || !(extId.upper().contains("WML 1.3//EN")
                    || extId.upper().contains("WML 1.2//EN")
                    || extId.upper().contains("WML 1.1//EN")
                    || extId.upper().contains("WML 1.1//ZH")   //for compatible cmcc BUG.
                    || extId.upper().contains("WML 1.0//EN"))))
          handleError(fatal, "Invalid DTD Public ID", lineNumber(), columnNumber());
        /***RK_ID:RK_WEBKIT. DEP_RK_ID:NULL. AUT:wangwf1@lenovo.com. DATE:2012-03-12. END***/
#endif
#if ENABLE(XHTMLMP)
        String dtdName = toString(name);
        if (extId == "-//WAPFORUM//DTD XHTML Mobile 1.0//EN"
            || extId == "-//WAPFORUM//DTD XHTML Mobile 1.1//EN"
        /***RK_ID:RK_WEBKIT. DEP_RK_ID:NULL. AUT:makun2@lenovo.com. DATE:2012-04-01. START***/
        /**
        *To fix bug:161503
        *In "3g.sina.com".The DOCTYPE is :
        * <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Strict//EN" "DTD/xhtml-strict.dtd">
        */
            || extId == "-//W3C//DTD XHTML 1.0 Strict//EN"
        /***RK_ID:RK_WEBKIT. DEP_RK_ID:NULL. AUT:makun2@lenovo.com. DATE:2012-04-01. END***/
            ){
