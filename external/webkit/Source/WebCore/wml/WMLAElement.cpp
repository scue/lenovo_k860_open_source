...
void WMLAElement::defaultEventHandler(Event* event)
{
    if (isLink()) {
	    ...
	    /***fix bug, value of input line is not post when send http request, that mean the url of http request is not include value of input line***/
            //handleLinkClick(event, document(), stripLeadingAndTrailingHTMLSpaces(getAttribute(HTMLNames::hrefAttr)), target(), event);
            handleLinkClick(event, document(), substituteVariableReferences(stripLeadingAndTrailingHTMLSpaces(getAttribute(HTMLNames::hrefAttr)), document(), WMLVariableEscapingEscape), target(), event);
            /***RK_ID:RK_WEBKIT. DEP_RK_ID:NULL. AUT:wangwf1@lenovo.com. DATE:2012-03-22. START***/
            return;
        }
    }
