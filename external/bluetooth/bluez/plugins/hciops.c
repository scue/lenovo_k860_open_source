...
/*Bluez will not work properly when connect&disconnect with headset very quickly, It looks like bluez kernel
 does not work properly when connect&disconnect quickly, so here disconnect all ACL connection intentional,
to make the repetition rate much less.*/
#ifndef HCI_DEV_ID
#define HCI_DEV_ID  0
#endif
...
/* lgh here we do not need to do hci0 up&down here
As when power on, no matter FM is on or off, we have arelady done hciconfig hci0 up in bluedroid, so do not need to do
up here. And When power off,  if FM is on, then we should not down hci0, and if FM is off, we will do it in bluedroid
anyway, so do not need to do down here
In conlusion, we do not need to do up&down here*/
static int hciops_set_powered(int index, gboolean powered)
{
    ...
    /*lgh always not power off*/
	if (powered == FALSE){
        uint8_t mode;

        DBG("hci%d set scan mode off", index);
        mode = SCAN_DISABLED;
        if (hci_send_cmd(dev->sk, OGF_HOST_CTL, OCF_WRITE_SCAN_ENABLE, 1, &mode) < 0)
            return -errno;
        cls_conn();
